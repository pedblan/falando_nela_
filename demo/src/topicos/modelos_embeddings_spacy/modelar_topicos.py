...
def preparar_docs_bertopic(df: pd.DataFrame, coluna: str) -> Tuple[pd.DataFrame, List[str]]:
    """
    Filtra o DataFrame para manter apenas documentos válidos na coluna de texto,
    e retorna os documentos prontos para o BERTopic.

    Args:
        df (pd.DataFrame): DataFrame original com todas as colunas.
        coluna (str): Nome da coluna de interesse.
    ...
    """
    df_filtrado = df[df[coluna].notnull() & df[coluna].str.strip().astype(bool)].copy()

    docs_validados = df_filtrado[coluna].tolist()
    print(f"Total de documentos válidos extraídos: {len(docs_validados)}")

    return df_filtrado, docs_validados
...
def treinar_vetorizador() -> CountVectorizer:
    ...
    Usa n-gramas e uma lista personalizada de stopwords para eliminar ruídos comuns em discursos parlamentares.
...
def representacao() -> Tuple[Dict[str, object], SentenceTransformer]:
    ...
    com base em análise morfossintática (POS) e diversidade semântica (MMR).

    Returns:
        Tuple[Dict[str, object], SentenceTransformer]:
            - Dicionário com representações ('Main': POS, 'Aspect1': MMR)
            - Modelo de embeddings nativo usado no BERTopic
    """
...
def main() -> None:
    """
    Executa o pipeline de modelagem de tópicos com BERTopic,
    usando embeddings nativos ('all-MiniLM-L6-v2') ao invés de FAISS ou OpenAI.

    Etapas:
    - Carregamento de dados
    - Filtragem por ano
    - Vetorização, clusterização e representação POS + MMR
    - Treinamento e redução de tópicos
    - Visualização e salvamento dos resultados
    """
    print("\n📥 Início do pipeline BERTopic\n")

    coluna = input("Digite a coluna de texto a ser analisada: ").strip()
